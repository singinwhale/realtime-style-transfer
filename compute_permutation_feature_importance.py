...
class PermutationFeatureImportanceData():
    ...
    def __init__(self):
        ...
        self.num_samples = 0
        self.num_samples_and_permutations = 0
...
if permutation_importance_data_cache_file_path.exists():
    ...
    print(permutation_importance_data.__dict__)
...
for i, sample in enumerate(progressify(preloaded_validation_dataset, position=0, desc="Sample")):
    if permutation_importance_data.num_samples > i:
        continue
    ...
    permutation_importance_data.num_samples_and_permutations = (i + 1) * (len(preloaded_validation_dataset) - 1)
    permutation_importance_data.num_samples = i + 1
...
log.info(
    f"Averaging the results for {permutation_importance_data.num_samples_and_permutations} samples and permutations")
for loss, channels in permutation_importance_data.channel_contributions.items():
    for channel, channel_value in channels.items():
        permutation_importance_data.channel_contributions[loss][channel] = (
                channel_value / permutation_importance_data.num_samples_and_permutations).numpy().item()

for loss, loss_value in permutation_importance_data.baseline_losses.items():
    permutation_importance_data.baseline_losses[loss] = (
                loss_value / permutation_importance_data.num_samples).numpy().item()

import pandas as pd

losses_summary = {
    loss.replace("_", " "): {channel: contribution / permutation_importance_data.baseline_losses[loss] * 100 for channel, contribution in
           channels.items()} for loss, channels in permutation_importance_data.channel_contributions.items()}

print(pd.DataFrame.from_dict(permutation_importance_data.channel_contributions))
print(pd.DataFrame.from_dict({'baseline':permutation_importance_data.baseline_losses}, orient='index'))

importance_frame = pd.DataFrame\
    .from_dict(losses_summary, orient="columns")\
    .sort_values(by="loss",ascending=False)
# importance_frame['Baseline'] = permutation_importance_data.baseline_losses
print(importance_frame)
style = importance_frame.style
style = style.format('{:.02f}\\%')
