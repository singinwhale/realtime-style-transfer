...
def predict_datapoint(validation_log_datapoint, training_log_datapoint, model: tf.keras.Model, callbacks=None):
    ...
    valiation_prediction_plot.imshow(tf.squeeze(model.predict(validation_log_datapoint, callbacks=callbacks, verbose=0)))
    training_prediction_plot.imshow(tf.squeeze(model.predict(training_log_datapoint, callbacks=callbacks, verbose=0)))
